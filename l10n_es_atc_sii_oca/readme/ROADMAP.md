...
**Motivo:** en 18.0 el IGIC canario va en **core** (`l10n_es` / `es_canary_pymes`), no en
módulo `l10n_es_igic` (ese módulo es de 16.0; ver #5050). Pendiente ampliar mapas
cuando estén en core los xmlids (`igic_r_1`, `igic_ISP1`, minorista, etc.).

| # | Entrega | Ficheros | Test a activar |
|---|---------|----------|----------------|
| 1.1 | Mapa ATC: `igic_r_1` → SFESB (+ ISP1, RE01, retail…) | `l10n.es.aeat.map.tax.line.tax.csv`, `atc_sii_map_data.xml` | `test_sale_igic_1_percent_petroleum` |
| 1.2 | Documentar requisito plan `es_canary_pymes` en tests | `readme/CONFIGURE.md` | `TestL10nEsAtcSiiPayloadBase` |

## Fase 2 — Nodos de payload en facturas (`account.move`) — entregado parcialmente

**Motivo:** esperar port a 18.0 del bloque DUA de [OCA/l10n-spain#5050](https://github.com/OCA/l10n-spain/pull/5050) (16.0, con `l10n_es_igic` separado).
...
| 2.9 | **Art. 25 REF** — `DatosArticulo25` claves 17/19 | Campos `sii_art25_*` + Lista L32 + inyección payload | `test_purchase_art25_clave_17_notarial` y relacionados |
