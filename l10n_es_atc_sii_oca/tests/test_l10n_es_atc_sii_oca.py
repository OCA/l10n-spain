...
class TestL10nEsAtcSii(TransactionCase):
    ...
    def test_01b_parse_aeat_vat_info_no_collision_atc_context(self):
        """Partners without VAT must not share AEAT id under ATC context."""
        ...
        atc_ctx = {"is_canary_tax_agency": True}
        self.assertEqual(
            partner_a.with_context(**atc_ctx)._parse_aeat_vat_info(),
            ("PT", "06", "PT_IDENTIFICATION_A"),
        )
        self.assertEqual(
            partner_b.with_context(**atc_ctx)._parse_aeat_vat_info(),
