...
class TestAtcSiiPayloadSales(TestL10nEsAtcSiiPayloadBase):
    ...
    def test_sale_igic_1_percent_petroleum(self):
        ...
        if not tax:
            self.skipTest(
                "account_tax_template_igic_r_1 not in chart "
                "(pending core l10n_es / es_canary_pymes)"
            )
    ...
    def test_sale_exempt_freezone_e4(self):
        """Exención zonas francas: CausaExencion E4."""
        self.product.sii_exempt_cause = "E4"
        move = self._create_atc_invoice(
            taxes=self._tax("igic_ex_0"),
            reg_key_code="01",
        )
        ...
        exenta = self._walk_payload(payload, "DetalleExenta")
        self.assertTrue(exenta)
        self.assertEqual(exenta[0]["CausaExencion"], "E4")

    def test_sale_exempt_zec_e6(self):
        """Exención ZEC: CausaExencion E6."""
        self.product.sii_exempt_cause = "E6"
        move = self._create_atc_invoice(taxes=self._tax("igic_re_ex"))
        payload = self._payload(move)
        exenta = self._walk_payload(payload, "DetalleExenta")
        self.assertEqual(exenta[0]["CausaExencion"], "E6")

    def test_sale_exempt_repep_e7(self):
        """Exención REPEP: CausaExencion E7."""
        self.product.sii_exempt_cause = "E7"
        move = self._create_atc_invoice(taxes=self._tax("igic_re_ex"))
        payload = self._payload(move)
        exenta = self._walk_payload(payload, "DetalleExenta")
        self.assertEqual(exenta[0]["CausaExencion"], "E7")

    def test_sale_exempt_other_e8(self):
        """Exención otros: CausaExencion E8."""
        self.product.sii_exempt_cause = "E8"
        move = self._create_atc_invoice(taxes=self._tax("igic_re_ex"))
        payload = self._payload(move)
        exenta = self._walk_payload(payload, "DetalleExenta")
        self.assertEqual(exenta[0]["CausaExencion"], "E8")
    ...
    def test_sale_art25_clave_19(self):
        """Art. 25 ventas clave 19: DatosArticulo25."""
        self._prepare_art25_product(tipo_bien="02", exempt_cause="E5")
        move = self._create_atc_invoice(
            taxes=self._tax("igic_re_ex"),
            reg_key_code="19",
            extra_vals=self._art25_invoice_vals(ref="ART25-SALE-19"),
        )
        payload = self._payload(move)
        factura = self._factura_expedida(payload)
        self.assertEqual(factura["ClaveRegimenEspecialOTrascendencia"], "19")
        self.assertTrue(self._walk_payload(payload, "DatosArticulo25"))
