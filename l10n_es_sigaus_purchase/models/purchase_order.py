...
class PurchaseOrder(models.Model):
    ...
    def _get_sigaus_line_vals(self, lines=False, **kwargs):
        ...
        if sigaus_vals.get("invoice_lines"):
            sigaus_vals["date_planned"] = self.env[
                "purchase.order.line"
    ...
    def copy(self, default=None):
        ...
        # 18.0: copy now works on multiple records — handle per-record
        copied = self.env["purchase.order"]
        for rec in self:
            rec_default = dict(default or {}) if default else None
            new_po = super(PurchaseOrder, rec.with_context(avoid_recursion=True)).copy(
                rec_default
            )
            copied |= new_po
        return copied
