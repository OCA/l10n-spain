...
class TestL10nEsDuaSii(common.SavepointCase):
    ...
    def setUpClass(cls):
        ...
        cls.fp_dua = cls.env["account.fiscal.position"].create(
            dict(
    ...
    def test_dua_sii(self):
        ...
        invoice.company_id.write(
            {"sii_enabled": True, "vat": "ESU2687761C",}
        )
        values = invoice._get_sii_invoice_dict_in()
        self.assertEqual(values["FacturaRecibida"]["TipoFactura"], "F5")
        self.assertEqual(
            values["FacturaRecibida"]["IDEmisorFactura"]["NIF"], "U2687761C"
        )
        self.assertEqual(values["FacturaRecibida"]["Contraparte"]["NIF"], "U2687761C")
        self.assertEqual(
