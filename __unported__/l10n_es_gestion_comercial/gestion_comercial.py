...
class account_journal(osv.osv):
    ...
    # a documentos bancarios, es decir que no abonan directamente la factura
    # sino que agrupan la deuda en un nuevo efecto cobrable
...
class account_voucher(osv.osv):
    ...
    def onchange_partner_id(self, cr, uid, ids, partner_id, journal_id, amount, currency_id, ttype, date, context=None):
        ...
        res = super(account_voucher, self).onchange_partner_id(
            cr, uid, ids, partner_id, journal_id, amount, currency_id, type, date)
        ...
        for inv in self.browse(cr, uid, ids, context=context):
            ...
            if inv.number:
                ...
                raise osv.except_osv(
                    _('Error !'), _('Please define a sequence on the journal !'))
            ...
            if inv.type in ('purchase', 'payment'):
                credit = currency_pool.compute(
                    cr, uid, current_currency, company_currency, inv.amount, context=context_multi_currency)
            ...
            if inv.type == 'sale':
                line_total = line_total - currency_pool.compute(
                    cr, uid, inv.currency_id.id, company_currency, inv.tax_amount, context=context_multi_currency)
            ...
            for line in inv.line_ids:
                ...
                if line.amount == line.amount_unreconciled:
                    ...
                    amount = currency_pool.compute(
                        cr, uid, current_currency, company_currency, line.untax_amount or line.amount, context=context_multi_currency)
                move_line = {
                    'journal_id': inv.journal_id.id,
                    'period_id': inv.period_id.id,
                ...
                if move_line.get('account_tax_id', False):
                    tax_data = tax_obj.browse(
                        cr, uid, [move_line['account_tax_id']], context=context)[0]
                    if not (tax_data.base_code_id and tax_data.tax_code_id):
                        raise osv.except_osv(_('No Account Base Code and Account Tax Code!'), _(
                            "You have to configure account base code and account tax code on the '%s' tax!") % (tax_data.name))
                sign = (
...
# el valor del comprante de pago si el efecto no esta relacionado
# directamente con una factura
...
# asociada el apunte, lo busca en el comprobante de pago... Si no esta en
# ninguno de los dos, lo deja en blanco.
        ...
        for rec in self.browse(cr, uid, ids, context):
            ...
            if invoice_id:
                ...
                voucher_id = voucher_obj.search(
                    cr, uid, [('move_id', '=', rec.move_id.id)], context=context)
                if voucher_id:
                    voucher = voucher_obj.browse(
                        cr, uid, voucher_id[0], context)
                    if voucher.payment_type:
                        result[rec.id] = (voucher.payment_type.id, self.pool.get(
                            'payment.type').browse(cr, uid, voucher.payment_type.id, context).name)
                    else:
                        result[rec.id] = (0, 0)
                else:
                    result[rec.id] = (0, 0)
...
# Sin modificaciones del original de momento... hay que hacer que
# encuentre los heredados del comprobante de cobro
...
# Creamos los metodos de busqueda para obtener los registros que tienen el
# check de efecto de gestión comercial marcado
