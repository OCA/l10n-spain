...
class res_partner(osv.osv):
    ...
    def write(self, cr, uid, ids, vals, context=None):
        ...
        if type(ids) != list:
            ids = [ids]
        domain = []
        # Buscamos la pos fiscal solamente si se cambia el valor al cif
        # intracomunitario válido
        ...
        for partner in self.browse(cr, uid, ids):
            ...
            if addr_ids:
                ...
                if address.country_id:
                    ...
                    country_domain.append(
                        ('country_ids', 'in', [address.country_id.id]))
                    fpos_ids = config_fpos_obj.search(cr, uid, country_domain)
                    if fpos_ids:
                        config_fpos = config_fpos_obj.browse(
                            cr, uid, fpos_ids[0])
                ...
                if address.state_id:
                    ...
                    state_domain.append(
                        ('state_ids', 'in', [address.state_id.id]))
...
class res_partner_address(osv.osv):
        ...
        if 'partner_id' in vals:
            ...
            if 'type' in vals and vals['type'] == 'invoice':
                if 'country_id' in vals and vals['country_id']:
                    country_domain = list(domain)
                    country_domain.append(
                        ('country_ids', 'in', [vals['country_id']]))
                    fpos_ids = config_fpos_obj.search(cr, uid, country_domain)
                    if fpos_ids:
                        config_fpos = config_fpos_obj.browse(
                            cr, uid, fpos_ids[0])
            ...
            partner_obj.write(
                cr, uid, vals['partner_id'], values, context=context)
    ...
    def write(self, cr, uid, ids, vals, context=None, update=True):
        ...
        result = super(res_partner_address, self).write(
        ...
        for addr in self.browse(cr, uid, ids, context=context):
            ...
            if addr.partner_id and addr.type == 'invoice':
                ...
                # Vies en domain
                domain.append(
                    ('vies_valid_vat', '=', addr.partner_id.valid_vies_vat))
                ...
                partner_obj.write(
                    cr, uid, addr.partner_id.id, values, context=context)
