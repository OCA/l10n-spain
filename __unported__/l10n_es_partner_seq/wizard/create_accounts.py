...
class create_accounts(orm.TransientModel):
    ...
    def _get_num_digits(self, cr, uid, data, context=None):
        ...
        property_ids = property_obj.search(cr, uid, [('name', '=', 'property_account_receivable'), (
            'company_id', '=', user.company_id.id), ('res_id', '=', False), ('value_reference', '!=', False)])
        num_digits = False
        if property_ids:
            prop = property_obj.browse(cr, uid, property_ids[0], context)
            if prop.value_reference:
                code = account_obj.read(
                    cr, uid, int(prop.value_reference.split(',')[1]), ['code'], context)['code']
                num_digits = len(code)
        return num_digits
    ...
    def create_accounts(self, cr, uid, ids, context=None):
        ...
        # Busca tipo cuenta de usuario rec
        res_ids = account_type_obj.search(
            cr, uid, [('code', '=', 'receivable')])
        acc_user_type_rec = res_ids and res_ids[0]
        # Busca tipo cuenta de usuario pay
        res_ids = account_type_obj.search(cr, uid, [('code', '=', 'payable')])
        ...
        def link_account(ref, parent_account, acc_type, acc_user_type, acc_property):
            ...
            acc_code = parent_account.code + \
                ref  # acc_code es el nuevo código de subcuenta
            if not account_obj.search(cr, uid, [('code', '=', acc_code)]):
                ...
                # Asocia la nueva subcuenta con el partner
        ...
        for partner in partner_obj.browse(cr, uid, context['active_ids'], context=context):
            ...
            # Solo nos interesa los dígitos del código
            ref = ''.join([i for i in ref if i in string.digits])
            if (ref.isdigit()):
                if partner.customer:
                    # longitud del código que aprovechamos
                    len_ref = form.num_digits - \
                        len(form.parent_receivable.code)
                    if len_ref > 0:
                        link_account(ref[-len_ref:].zfill(len_ref), form.parent_receivable,
                                     'receivable', acc_user_type_rec, 'property_account_receivable')

                if partner.supplier:
                    # longitud del código que aprovechamos
                    len_ref = form.num_digits - len(form.parent_payable.code)
                    if len_ref > 0:
                        link_account(ref[-len_ref:].zfill(len_ref), form.parent_payable,
                                     'payable', acc_user_type_pay, 'property_account_payable')
