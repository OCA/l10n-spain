...
class lopd_responsable(osv.osv):
    ...
    def datos_empresa(self, cr, uid, ids, dato):
        datos = self.pool.get('res.partner.address').search(
            cr, uid, [('partner_id', '=', 1)])
        datos = self.pool.get('res.partner.address').read(cr, uid, datos[0], [
            'street', 'zip', 'localidad', 'city', 'state_id', 'country_id', 'phone', 'fax', 'email'])
        v = {'domicilio': datos['street'], 'cp': datos['zip'], 'localidad': datos['city'], 'provincia': datos[
            'state_id'], 'pais': datos['country_id'], 'tlf': datos['phone'], 'fax': datos['fax'], 'email': datos['email'], }
        return {'value': v}
