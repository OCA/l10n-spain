...
class state_extension(osv.osv):
    ...
    _sql_constraints = [
        ...
        ('nc_unico', 'unique (name, country_id)',
         'La provincia ya existe en el pais'),
        ('cdc_unico', 'unique (code, country_id)',
         'No puede haber códigos repetidos en un pais'),
    ]
...
class lopd_departamentos(osv.osv):
    ...
    _defaults = {'locales': lambda self, cr, uid, context: context[
        'locales'] if context and 'locales' in context else None}
    _sql_constraints = [
        ('id_unico', 'unique (id)', '¡El ID del recurso debe ser único!'), ]
...
class lopd_sos(osv.osv):
    ...
    _sql_constraints = [
        ('id_unico', 'unique (id)', '¡El ID del equipo debe ser único!'), ]
    _defaults = {
        'sca': 2,
        'lif': 2,
...
class lopd_sos_ext(osv.osv):
    _name = 'lopd.sos'
    _inherit = 'lopd.sos'
    _columns = {'id_programas': fields.many2many(
        'lopd.programas', 'lopd_rel_prso', 'so_id', 'pr_id', 'Programas asociados'), }
lopd_sos_ext()
...
class lopd_departamentos_ext(osv.osv):
    ...
    _columns = {'id_zars': fields.one2many(
        'lopd.zar', 'departamento', 'Zar relacionadas')}
...
class lopd_zar_ext(osv.osv):
    _name = 'lopd.zar'
    _inherit = 'lopd.zar'
    _columns = {'id_soporte': fields.one2many(
        'lopd.soportes', 'zar', 'Soportes')}
lopd_zar_ext()
