...
def get_configuration(cr, uid, ids, context=None):
    ...
    if obj_company.cuenta_ss_empresa:
        # Como las cuentas son obligatorias si esta definida esta cuenta
        # estarán todas definidas
        ...
        raise osv.except_osv(_('No hay una configuración de cuentas activa para la compañia ') + company_name, _(
            'Debe configurar las cuentas!\nPor favor configure las cuentas en el menú de configuración de la compañia: ') + company_name)
...
class hr_nomina(osv.osv):
    ...
    def comprueba_mes(self, fecha_anticipo, fecha_nomina):
        ...
        dateAnterior = time.strptime(
            (dateNomina - datetime.timedelta(nomina[2] + 1)).strftime('%Y-%m-%d'), '%Y-%m-%d')

        # Si se solicito anticipo el mes pasado
        if (anticipo[0] == dateAnterior[0]) and (dateAnterior[1] == anticipo[1]):
            return True
        else:
            return False
    ...
    def comprueba_anticipo(self, cr, uid, ids, fechaNomina, empleado_id):
        ...
        ################################################################
        anticipo_ids = anticipo_obj.search(
    ...
    def confirmar_nomina(self, cr, uid, ids, *args):
        ...
        for nom in self.browse(cr, uid, ids):
            ...
            period_ids = account_period_obj.search(cr, uid, [('date_start', '<=', fechaNomina or time.strftime(
                '%Y-%m-%d')), ('date_stop', '>=', fechaNomina or time.strftime('%Y-%m-%d'))])
            if len(period_ids):
                periodo_id = period_ids[0]
            else:
                raise osv.except_osv(_('No existe un periodo para esa fecha de nomina!'), _(
                    'No se pueden generar nóminas cuya fecha esté en un periodo que no existe, \nsi desea generarla por favor cree el periodo contable correspondiente.'))

            referencia = str(
                numero) + ' : ' + str(nom.employee_id.name) + ' - ' + str(fechaNomina)
            if nom.extra:
                referencia = "Paga Extra: " + \
                    str(nom.employee_id.name) + ' - ' + str(fechaNomina)
            move_val = {
                'ref': referencia,
            ...
            if not cuenta_id:
                raise osv.except_osv(_('No existe una cuenta configurada para el empleado!'), _(
                    'Por favor configure una cuenta en la ficha del empleado en la que generar los asientos de la nómina.'))
            retencion_irpf = (nom.retribucion_bruta * nom.irpf) / 100
            anticipo = self.comprueba_anticipo(
                cr, uid, ids, fechaNomina, nom.employee_id.id)

            sueldo_neto = nom.retribucion_bruta - \
                retencion_irpf - nom.ss_trabajador
            ...
            self.write(
                cr, uid, ids, {'state': 'confirmada', 'asiento_nomina_confirmada': move_id})
        return True
    ...
    def pagar_nomina(self, cr, uid, ids, *args):
        ...
        for nom in self.browse(cr, uid, ids):
            ...
            referencia = nom.numero + ' : Pago ' + \
                nom.employee_id.name + ' - ' + fechaNomina
            if nom.extra:
                referencia = "Pago de Paga Extra: " + \
                    nom.employee_id.name + ' - ' + fechaNomina
            move = {'ref': referencia, 'journal_id': journal_id,
                    'date': fechaNomina, 'period_id': periodo_id}
            ...
            self.write(
                cr, uid, ids, {'state': 'pagada', 'asiento_nomina_pagada': move_id})
            account_move_obj.write(cr, uid, [move_id], {'date': fechaNomina})
            account_move_obj.post(cr, uid, [move_id])
        return True
    ...
    def cancelar_nomina(self, cr, uid, ids, *args):
        for nom in self.browse(cr, uid, ids):
            ...
            if nom.state == 'confirmada':
                acc_obj.button_cancel(
                    cr, uid, [nom.asiento_nomina_confirmada.id])
...
class hr_anticipo(osv.osv):
    ...
    def confirmar_anticipo(self, cr, uid, ids, *args):
        ...
        for anticipo in self.browse(cr, uid, ids):
            ...
            # PERIODO
            period_ids = account_period_obj.search(cr, uid, [('date_start', '<=', fecha_anticipo or time.strftime(
                '%Y-%m-%d')), ('date_stop', '>=', fecha_anticipo or time.strftime('%Y-%m-%d'))])
            if len(period_ids):
                periodo_id = period_ids[0]
            referencia = 'Anticipo: ' + \
                anticipo.employee_id.name + ' - ' + fecha_anticipo
            ...
            self.write(
                cr, uid, ids, {'state': 'confirmado', 'asiento_anticipo': move_id})
            account_move_obj.write(
                cr, uid, [move_id], {'date': fecha_anticipo})
        return True
