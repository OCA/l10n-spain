...
class AccountInvoice(models.Model):
                ...
                if self.tbai_refund_origin_ids:
                    ...
                    for refund_origin_id in self.tbai_refund_origin_ids:
                        refund_id_dicts.append(
                            (0, 0, {'number_prefix': refund_origin_id.number_prefix,
    ...
    def invoice_validate(self):
        ...
        def validate_refund_invoices():
            for invoice in refund_invoices:
                ...
                if not invoice.refund_invoice_id \
                    and \
                    not \
                        invoice.tbai_refund_origin_ids:
                    valid_refund = False
                ...
                if invoice.refund_invoice_id.tbai_invoice_id:
                    refund_state = invoice.refund_invoice_id.tbai_invoice_id.state
                    pending_state = TicketBaiInvoiceState.pending.value
                    sent_state = TicketBaiInvoiceState.sent.value
                    valid_refund_states = [pending_state, sent_state]
                    valid_refund = refund_state in valid_refund_states
                    if not valid_refund:
                        error_refund_msg = _("Some of the original invoices have "
                                             "related tbai invoices in "
                                             "inconsistent state please fix them "
                                             "beforehand.")
                if valid_refund and invoice.refund_invoice_id.tbai_cancellation_id:
                    valid_refund = False
                    error_refund_msg = _("Some of the original invoices have related"
                                         " tbai cancelled invoices")
                if not valid_refund:
        ...
        tbai_invoices |= self.sudo().filtered(lambda x:
                                              x.tbai_enabled and
                                              'out_invoice' == x.type)
        refund_invoices = self.sudo().filtered(lambda x:
                                               x.tbai_enabled and
                                               'out_refund' == x.type and
                                               x.tbai_refund_type ==
