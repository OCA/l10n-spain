...
class TicketBAITaxAgency(models.Model):
    ...
    version = fields.Char(string='TicketBAI version', compute='_compute_ticketbai_version', store=True)
    qr_base_url = fields.Char(string='QR Base URL', compute='_compute_ticketbai_version', store=True)
    tax_agency_version_ids = fields.One2many(comodel_name='tbai.tax.agency.version', inverse_name='tbai_tax_agency_id')
    ...
    def _compute_ticketbai_version(self):
        for record in self:
            ...
            tax_agency_version = self.env['tbai.tax.agency.version'].search(search_domain)
            record.version = tax_agency_version.version
            record.qr_base_url = tax_agency_version.qr_base_url

    def tbai_get_value_IDVersionTBAI(self, **kwargs):
        """ V 1.1
...
class TicketBAITaxAgencyVersion(models.Model):
    ...
    def _check_ticketbai_version(self):
        if len(self.version) not in range(1, 6):
            raise exceptions.ValidationError(_("TicketBAI version max size is 5 characters."))

    @api.one
    @api.constrains('date_from', 'date_to')
    def _unique_date_range(self):
        # Based in l10n_es_aeat module
        domain = [('id', '!=', self.id), ('tbai_tax_agency_id', '=', self.tbai_tax_agency_id.id)]
    ...
    tbai_tax_agency_id = fields.Many2one(comodel_name='tbai.tax.agency', required=True, ondelete='restrict')
