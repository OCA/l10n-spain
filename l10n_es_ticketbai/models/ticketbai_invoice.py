    ...
    def _compute_tbai_identifier(self):
        if self.signature_value:
            ...
            if tbai_identifier_length_without_crc != len(tbai_identifier_without_crc):
                ...
                ) % (self.invoice_id.number, tbai_identifier_without_crc,
                     tbai_identifier_length_without_crc))
            ...
            if tbai_identifier_length_with_crc != len(tbai_identifier_with_crc):
                raise exceptions.ValidationError(_(
                    "TicketBAI Invoice %s Error: TBAI identifier with CRC-8 %s should "
                    "be %d characters long!"
                ) % (self.invoice_id.number, tbai_identifier_with_crc,
                     tbai_identifier_length_with_crc))
    ...
    def _compute_tbai_qr(self):
        ...
        if self.tbai_identifier:
            ...
            qr_url_without_crc = "%s?%s" % (
                self.company_id.tbai_tax_agency_id.qr_base_url,
