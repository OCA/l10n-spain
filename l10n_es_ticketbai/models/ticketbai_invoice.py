...
class TicketBAIInvoice(models.AbstractModel):
    ...
    def _get_ticketbai_api(self):
        self.ensure_one()
        if self.company_id.tbai_test_enabled:
            url = self.company_id.tbai_tax_agency_id.test_rest_url_customer_invoice
        else:
            raise exceptions.ValidationError(_(
                "TicketBAI production servers not available!"))
        return TicketBaiApi(
            url, self.company_id.tbai_certificate_id.public_key,
            self.company_id.tbai_certificate_id.private_key)
