...
class TicketBAIInvoiceRefundOrigin(models.Model):
    ...
    account_refund_invoice_id = fields.Many2one(comodel_name='account.invoice',
                                                ...
                                                required=True)
    number = fields.Char(required=True, help='The number of the invoice name e.g. if'
                                             ' the invoice name is INV/2021/00001'
                                             ' then the number is 00001')
    number_prefix = fields.Char(default='', help='Number prefix of this invoice name'
                                                 ' e.g. if the invoice name is'
                                                 ' INV/2021/00001 then the prefix is '
                                                 'INV/2021/, '
                                                 'ending back slash included')
    expedition_date = fields.Char(required=True, help="Expected string date format:"
                                                      " %dd-%mm-%yyyy")
    ...
    def _check_account_invoice_exists(self):
        for record in self:
            ...
            if invoice_number and record.expedition_date:
                ...
                account_invoice = \
                    self.sudo().env['account.invoice'].search(domain_invoice)
                if account_invoice:
                    raise exceptions.ValidationError(_(
