...
class TestL10nEsTicketBAI(common.TransactionCase):
    ...
    def create_account_billing(self):
        return self.env['res.users'].with_context({'no_reset_password': True}).create({
            ...
            'groups_id': [(6, 0, [self.group_user.id, self.res_users_account_billing.id, self.partner_manager.id])]
        })
    ...
    def create_account_manager(self):
        return self.env['res.users'].with_context({'no_reset_password': True}).create({
            ...
            'groups_id': [(6, 0, [self.group_user.id, self.res_users_account_manager.id, self.partner_manager.id])]
        })
    ...
    def setUp(self):
        ...
        catalog_path = f"file:{pathname2url(os.path.join(schemas_dirpath, 'catalog.xml'))}"
        os.environ['XML_CATALOG_FILES'] = catalog_path
        # Load XSD file with XADES imports
        test_xml_customer_invoice_filepath = os.path.abspath(
            os.path.join(schemas_dirpath, '%s/test_ticketBai V1-1.xsd' % schemas_version_dirname))
        ...
        test_xml_customer_cancellation_filepath = os.path.abspath(os.path.join(
            schemas_dirpath, '%s/test_Anula_ticketBai V1-1.xsd' % schemas_version_dirname))
        ...
        self.main_company.tbai_tax_agency_id = self.env.ref('l10n_es_ticketbai.tbai_tax_agency_gipuzkoa').id
        ...
        self.main_company.tbai_developer_id = self.env.ref("l10n_es_ticketbai.res_partner_binovo").id
        ...
        self.customer_extracommunity = self.env.ref('l10n_es_ticketbai.res_partner_yamaha_jp')
        ...
        self.res_users_account_billing = self.env.ref('account.group_account_invoice')  # Billing
        self.res_users_account_manager = self.env.ref('account.group_account_manager')  # Billing Manager
        ...
        self.vat_exemption_E2 = self.env.ref('l10n_es_ticketbai.tbai_vat_exemption_E2')  # Exportaciones de mercancías
        self.vat_exemption_E5 = self.env.ref('l10n_es_ticketbai.tbai_vat_exemption_E5')  # Entregas a otro estado miembro
        self.vat_exemption_E6 = self.env.ref('l10n_es_ticketbai.tbai_vat_exemption_E6')  # Otros
        self.tax_21b = self.env['account.tax'].search([('description', '=', 'S_IVA21B')])  # Bienes
        self.tax_10s = self.env['account.tax'].search([('description', '=', 'S_IVA10S')])  # Servicios
        self.tax_irpf_15 = self.env['account.tax'].search([('description', '=', 'S_IRPF15')])  # Retenciones a cuenta IRPF 15%
        self.tax_req14 = self.env['account.tax'].search([('description', '=', 'S_REQ014')])  # 1.4% Recargo Equivalencia Ventas
        self.tax_req52 = self.env['account.tax'].search([('description', '=', 'S_REQ52')])  # 5.2% Recargo Equivalencia Ventas
        self.tax_iva0_ic = self.env['account.tax'].search([('description', '=', 'S_IVA0_IC')])  # Bienes intracomunitarios
        self.tax_iva0_sp_i = self.env['account.tax'].search([('description', '=', 'S_IVA0_SP_I')])  # Servicios intracomunitarios
        self.tax_iva0_e = self.env['account.tax'].search([('description', '=', 'S_IVA0_E')])  # Bienes extracomunitarios
        self.tax_iva0_sp_e = self.env['account.tax'].search([('description', '=', 'S_IVA_SP_E')])  # Servicios extracomunitarios
        ...
        self.fiscal_position_national = self.env['account.fiscal.position'].create({
            ...
            'tbai_vat_regime_key': self.env.ref('l10n_es_ticketbai.tbai_vat_regime_07').id,  # Régimen especial criterio de caja
            'tbai_vat_regime_key2': self.env.ref('l10n_es_ticketbai.tbai_vat_regime_05').id,  # Régimen especial de agencias de viajes
            'tbai_vat_regime_key3': self.env.ref('l10n_es_ticketbai.tbai_vat_regime_01').id  # Régimen general
        })
        self.fiscal_position_surcharge = self.env['account.fiscal.position'].create({
            'name': 'TBAI Fiscal Position - Surcharge',
            'tbai_vat_regime_key': self.env.ref('l10n_es_ticketbai.tbai_vat_regime_01').id,
            'tax_ids': [
                (0, 0, {
        ...
        self.fiscal_position_e = self.env['account.fiscal.position'].create({
            ...
            'tbai_vat_regime_key': self.env.ref('l10n_es_ticketbai.tbai_vat_regime_02').id,
            'tbai_vat_exemption_ids': [
                (0, 0, {
