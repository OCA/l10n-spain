...
class AccountMoveline(models.Model):
    ...
    def _onchange_product_id(self):
        ...
        for line in self:
            # if line.is_plastic_tax == False:
            #     if line.product_id.is_plastic_tax == True:
            #         line.update({"is_plastic_tax": True})
            if line.is_plastic_tax:
                # LINEAS FABRICANTES SIN STOCK ACTUAL
                line.update({"product_plastic_tax_description": line.move_id.name})
                if self.move_id.company_plastic_type == "manufacturer":
                    ...
                    if self.move_id.move_type == "in_refund":
                        line.update({"product_plastic_concept_manufacturer": "4"})
                if self.move_id.company_plastic_type == "acquirer":
                    if (
