...
class ImportPurchaseOrder(models.TransientModel):
    ...
    def import_button(self):
        ...
        if not extension:
            raise UserError(_("El fichero debe tener extensión .csv, .xls o .xlsx"))
        ...
        for line in archive_lines:
            move_id = int(line.get(u'Número Referencia Interno', 0))
            account_number = anti_float(line.get(u'Número Asiento', ""))
            account_type = anti_float(line.get(u'Tipo Asiento', ""))
    ...
    def valid_columns_keys(self, archive_lines):
        ...
        if not 'Número Asiento' in columns:
            text += "\n[ Número Asiento ]"
        if not u'Tipo Asiento' in columns:
            text += "\n[ Tipo Asiento ]"
        if not u'Número Referencia Interno' in columns:
    ...
    def csv_validator(self, xml_name):
        ...
        return extension if extension == '.csv' or extension == '.xls' or extension == '.xlsx' else False
