        ...
        cls.profile = cls.env["account.asset.profile"].create(
            {
                ...
                "account_expense_depreciation_id": cls.env.ref(
                    f"l10n_es.{cls.company.id}_account_common_681"
                ).id,
                "account_asset_id": cls.env.ref(
                    f"l10n_es.{cls.company.id}_account_common_213"
                ).id,
                "account_depreciation_id": cls.env.ref(
                    f"l10n_es.{cls.company.id}_account_common_2813"
                ).id,
                "journal_id": cls.journal.id,
                "method_number": 3,
            }
        )
        tax = cls.env.ref(f"l10n_es.{cls.company.id}_account_tax_template_p_iva0_nd")
        line_vals = {
