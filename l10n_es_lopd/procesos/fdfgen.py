...
def crear_fdf(datos):
	...
	soporte = "FICHERO XML SIN FIRMA" #Soporte de la solicitud y modo de presentación
	...
	#TODO: Ojo, localidad en la que se firma el impreso, se define igual a la localidad de notificación
	...
	# id_notific: Dirección electrónica del servicio de notificaciones. Opcional. En los envíos realizados sin firma digital vendrá siempre vacío. Si el envío se hace con firma digital y se ha indicado en el elemento anterior (forma) un valor '2', sólo se deberá cumplimentar cuando la suscripción al Servicio de Notificaciones Telemáticas Seguras se haya realizado con un certificado de persona jurídica. Se consignará, en ese caso, la Dirección Electrónica Única (DEU) asignada por dicho Servicio. Cuando no se cumplimente, el servicio Web obtendrá la DEU a partir del los datos del NIF, nombre y apellidos de la persona física correspondiente al certificado con el que se ha firmado. Hasta 70 caracteres.
