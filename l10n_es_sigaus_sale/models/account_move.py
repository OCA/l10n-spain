...
class AccountMove(models.Model):
    ...
    def manage_sale_sigaus_lines(self):
        ...
        for sigaus_line in sigaus_lines:
            ...
            lines_from_order = self.invoice_line_ids.filtered(
                lambda a, order_id=order_id: a.sale_line_ids.order_id == order_id
                and a.product_id
        ...
        if len(sigaus_lines) > 1:
            for sigaus_line in sigaus_lines.filtered(
                ...
                sigaus_line.name = (
                    f"{sigaus_line.sale_line_ids.order_id.name}: " f"{sigaus_line.name}"
                )
    ...
    def apply_sigaus(self):
        for rec in self.filtered(
            lambda a: a.is_sigaus and a.sigaus_is_date and a.is_sale_document()
        ):
