...
class SaleOrder(models.Model):
    ...
    def apply_sigaus(self):
        for rec in self.filtered(
            ...
            # Buscar si ya existe UNA línea SIGAUS
            existing_sigaus_line = rec.order_line.filtered(lambda line: line.is_sigaus)

            # Eliminar línea SIGAUS existente para evitar duplicados
            if existing_sigaus_line:
                existing_sigaus_line.with_context(avoid_recursion=True).unlink()

            # Calcular valores SIGAUS (total de todas las aportaciones)
            sigaus_vals = rec._get_sigaus_line_vals()

            # Crear la nueva línea SIGAUS con valores actualizados
            self.env["sale.order.line"].with_context(avoid_recursion=True).create(
                sigaus_vals
            )

    def write(self, vals):
        res = super().write(vals)
        # Evitar reentradas cuando venimos de crear/actualizar líneas SIGAUS
        ...
        ).with_context(avoid_recursion=True)._delete_sigaus()
        return res
