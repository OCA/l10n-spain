...
def check_str_percentage(field, value, no_digits=3, no_decimal_digits=2):
    ...
    if not 0.0 <= float(value) <= 100.0:
        raise exceptions.ValidationError(_(
            "%s value must be a positive percentage.") % field)
...
def check_spanish_vat_number(field, value):
    ...
    match_res = re.match(pattern, value)
    if not match_res or (
            match_res and match_res.group(0) != value) or 9 != len(value):
...
def check_date(field, value):
    ...
    match_res = re.match(pattern, value)
    if not match_res or (
            match_res and match_res.group(0) != value) or 10 != len(value):
        raise exceptions.ValidationError(_("Invalid %s format '%s'.") % (field, value))
...
def check_hour(field, value):
    ...
    match_res = re.match(pattern, value)
    if not match_res or (
            match_res and match_res.group(0) != value):
        raise exceptions.ValidationError(_("Invalid %s format '%s'.") % (field, value))
