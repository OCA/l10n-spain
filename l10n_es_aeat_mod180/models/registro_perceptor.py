...
class RegistroPerceptor(models.Model):
    ...
    report_id = fields.Many2one('l10n.es.aeat.mod180.report',
                                string='AEAT 180 Report',
                                ondelete="cascade")
    partner_id = fields.Many2one('res.partner', string='Empresa')
    informacion_catastral_id = fields.Many2one('informacion.catastral',
                                               string='Información catastral')
    signo = fields.Selection(selection=[(' ', "Positivo"), ('N', "Negativo")],
                             string='Signo Base Retenciones')
    base_retenciones = fields.Float(string='Base retenciones e ingresos a cuenta',
                                    digits=(13, 2))
    cuota_retenciones = fields.Float(string='Retenciones e ingresos a cuenta',
                                     digits=(13, 2))
    porcentaje_retencion = fields.Float(string='% Retención', digits=(2, 2))
    ejercicio_devengo = fields.Integer(string='Ejercicio Devengo')
    base_move_line_ids = fields.Many2many('account.move.line',
                                          'reg_perceptor_base_move_line_rel',
                                          'reg_perceptor_id',
                                          'move_line_id',
                                          string='Apuntes contable de base')
    cuota_move_line_ids = fields.Many2many('account.move.line',
