...
class L10nEsAeatMod180Report(models.Model):
    ...
    def default_get(self, field_list):
        ...
        if res.get('company_id', False):
            ...
            bank_ids = company.partner_id.bank_ids
            if bank_ids:
                res.update({
    ...
    def _crear_registros_percetores(self):
        ...
        if tax_code_map:
            taxes = tax_code_map.map_line_ids.mapped('tax_ids')
            report_taxes = self.get_taxes_from_templates(taxes)
            if report_taxes:
                taxObj = self.env['account.tax']
                perceptorObj = self.env['registro.perceptor']
                mls = self.tax_line_ids.mapped('move_line_ids')
                partners = mls.mapped('partner_id').ids
                unique_partner_ids = list(set(partners))
                info_cat_ids = mls.mapped('informacion_catastral_id').ids
                unique_catastral_ids = list(set(info_cat_ids))
                tax_ids = mls.mapped('tax_ids') + mls.mapped('tax_line_id')
                ...
                for partner_id in unique_partner_ids:
                    for catastral_id in unique_catastral_ids:
                        for tax_id in unique_tax_ids:
                            ...
                            if lines:
                                bd = sum(lines.filtered(
                                    lambda aml: tax_id in aml.tax_ids.ids).
                                    mapped('debit'))
                                bc = sum(lines.filtered(
                                    lambda aml: tax_id in aml.tax_ids.ids).
                                    mapped('credit'))
                                base = bd - bc
                                b_lines = lines.filtered(
                                    lambda aml: tax_id in aml.tax_ids.ids)
                                cc = sum(lines.filtered(
                                    lambda aml: tax_id == aml.tax_line_id.id).
                                    mapped('credit'))
                                cd = sum(lines.filtered(
                                    lambda aml: tax_id == aml.tax_line_id.id).
                                    mapped('debit'))
                                cuota = cc - cd
                                c_lines = lines.filtered(
                                    lambda aml: tax_id == aml.tax_line_id.id)
                                ...
                                perceptorObj.create({
                                    ...
                                    'base_move_line_ids': [[6, False, b_lines.ids]],
                                    'cuota_move_line_ids': [[6, False, c_lines.ids]],
                                })
