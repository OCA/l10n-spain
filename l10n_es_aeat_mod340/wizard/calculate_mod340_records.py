    ...
    def _calculate_records(self, cr, uid, ids, context=None, recalculate=True):
        ...
        if len(account_period_ids) == 0:
            raise orm.except_orm(
                _('Error'),
                _("The periods selected don't belong to the fiscal year "
                  "%s") % (mod340.fiscalyear_id.name))
        ...
        del_ids = invoices340_rec.search(
            cr, uid, [('mod340_id', '=', mod340.id)])
        if del_ids:
        ...
        for invoice in invoice_obj.browse(cr, uid, invoice_ids, context):
            ...
            if include:
                ...
                if invoice.type in ("in_invoice", "in_refund"):
                    invoice_created = invoices340_rec.create(cr, uid,
                                                             values)
                tot_tax_invoice = 0
                check_tax = 0
                check_base = 0
