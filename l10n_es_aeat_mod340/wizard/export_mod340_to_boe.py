...
class l10n_es_aeat_mod340_export_to_boe(orm.TransientModel):
    ...
    def _formatDate(self, date_str):
        #TODO: mover este método a l10n.es.aeat.report.export_to_boe
        if not date_str:
            return '0'*8
    ...
    def _get_formatted_invoice_issued(self, cr, uid, report, invoice_issued):
        ...
        Format of the record:
            ...
            96-98       Blancos
            99          Clave tipo de libro.
            100         Clave de operación. Recuperado de los impuestos de la factura.
                        Constante 'C' para varios tipos de IVA.
            ...
            366-370     Tipo recargo de equivalencia.
            371-384     Cuota recargo de equivalencia.
        ...
        for tax_line in invoice_issued.tax_line_ids:
            ...
            text += 3 * ' '
            # Clave tipo de libro.                                          
            text += self._formatString(invoice_issued.ledger_key, 1)                                                       
            # Clave de operación
            if invoice_issued.invoice_id.origin_invoices_ids:               
                clave_operacion = 'D'
            elif len(invoice_issued.tax_line_ids) > 1: clave_operacion = 'C'
            elif invoice_issued.invoice_id.is_ticket_summary == 1: clave_operacion = 'B'
            else: clave_operacion = self._formatString(invoice_issued.operation_key, 1) 
            text += clave_operacion
            
            text += self._formatDate(invoice_issued.invoice_id.date_invoice)
            text += self._formatDate(invoice_issued.invoice_id.date_invoice)
            ...
            # Si una rectificativa lleva varios tipos de impuesto, 
            # se debe declarar con clave D y registros 1 (como si fueran facturas individuales)
            if clave_operacion == 'D':
            ...
            text += self._formatNumber(tax_line.surcharge_percentage, 3, 2)
            # Couta del recargo de equivalencia 
            text += self._formatNumber(tax_line.surcharge_amount, 11, 2,True)
    ...
    def _get_formatted_invoice_received(self, cr, uid, report, invoice_received):
        ...
        Format of the record:
            ...
            96-98       Blancos
            99          Clave tipo de libro.
            100         Clave de operación. Constante ' ' para un solo tipo de IVA. Recuperado de los impuestos de la factura.
                        Constante 'C' para varios tipos de IVA.
            101-108     Fecha de expedición
        ...
        for tax_line in invoice_received.tax_line_ids:
            ...
            text += 3*' '   
            # Clave tipo de libro.                 
            ledger_key = invoice_received.ledger_key
            if ledger_key in ('I','J'):
                ledger_key = 'R'                       
            text += self._formatString(ledger_key, 1)
            # Clave de operación
            if len(invoice_received.tax_line_ids) > 1 and invoice_received.operation_key not in ('P','I'): text += 'C'              
            else: text += self._formatString(invoice_received.operation_key, 1)  
            # Fecha de expedición
            text += self._formatDate(invoice_received.invoice_id.date_invoice)
            # Fecha de operación   
            text += self._formatDate(invoice_received.invoice_id.date_invoice)  
            #Tipo impositivo
            text += self._formatNumber(tax_line.tax_percentage * 100, 3, 2) 
    ...
    def _get_formatted_invoice_investment(self, cr, uid, report, invoice_investment):
        """
        Returns a type 2, investment goods invoice, formated record

        Format of the record:
            Tipo de Registro 2 – Registro de facturas de bienes de inversión
            Posiciones  Descripción
            1           Tipo de Registro
            2-4         Modelo Declaración
            5-8         Ejercicio
            ...
            79-95       Número de identificación fiscal en el país de
                        residencia. TODO de momento blancos.
            96-98       Blancos
            99          Clave tipo de libro. 'I' o 'J' (IGIC), según impuesto.
            100         Clave de operación. Recuperado de los impuestos de la factura.
                        Constante 'C' para varios tipos de IVA.
            101-108     Fecha de expedición
            109-116     Fecha de operación. Se consigna la misma que
            ...
            236-238     Prorrata anual definitiva
            239-252     Regularización anual de la deducción TODO
            253-292     Identificación de la entrega
            293-306     Regularización anual de la deducción efectuada TODO
            307-314     Fecha de inicio de la utilización
            315-331     Identificación del bien
            332-500     BLANCO

        """
        text = ''
        for tax_line in invoice_investment.tax_line_ids:
            # Tipo de Registro
            text += '2'
            # Modelo Declaración
            text += '340'
            ...
            text += self._formatString(report.company_vat, 9)
            # NIF del declarado        
            if invoice_investment.partner_country_code == 'ES': 
                text += self._formatString(invoice_investment.partner_vat, 9)
            else: text += self._formatString(' ', 9)
            # NIF del representante legal
            text += self._formatString(invoice_investment.representative_vat, 9)
            # Apellidos y nombre, razón social o denominación del declarado
            text += self._formatString(invoice_investment.partner_id.name, 40)
            # Código país
            text += self._formatString(invoice_investment.partner_country_code, 2)
            # Clave de identificación en el país de residencia
            text += self._formatNumber(invoice_investment.partner_id.vat_type, 1)
            # Número de identificación fiscal en el país de residencia.   
            if invoice_investment.partner_country_code != 'ES':
                text += self._formatString(invoice_investment.partner_country_code, 2)
                text += self._formatString(invoice_investment.partner_vat, 15)
            else: text += 17 * ' '
            # Blancos
            text += 3 * ' '
            # Clave tipo de libro. 'I' o 'J' (IGIC), según impuesto.
            text += self._formatString(invoice_investment.ledger_key, 1)                         
            # Clave de operación
            if len(invoice_investment.tax_line_ids) > 1 and invoice_investment.operation_key not in ('P','I'): text += 'C'              
            else: text += self._formatString(invoice_investment.operation_key, 1) 
            
            text += self._formatDate(invoice_investment.invoice_id.date_invoice)
            text += self._formatDate(invoice_investment.invoice_id.date_invoice)
            #Tipo impositivo
            text += self._formatNumber(tax_line.tax_percentage * 100, 3, 2)
            # Base imponible
            ...
            text += ' '+self._formatNumber(0, 11, 2)
            # Identificación de la factura  
            text += self._formatString(invoice_investment.invoice_id.number, 40)
            # Número de registro
            sequence_obj = self.pool.get('ir.sequence')
            text += self._formatString(sequence_obj.get(cr, uid, 'mod340'), 18)
            #Prorrata anual definitiva
            text += self._formatNumber(invoice_investment.prorata, 3, 0)
            #Regularización anual de la deducción TODO
            text += ' ' + '0'*13
            #Identificación de la entrega
#             Se consignarán la identificación de la factura que
#             corresponde en el LIBRO REGISTRO DE FACTURAS
#             EXPEDIDAS que recoge la entrega.
#             Esto qué es? la referencia del proveedor??
            text += self._formatString(invoice_investment.invoice_id.reference, 40)
            #Regularización de la deducción efectuada (transmisiones) TODO
            text += ' ' + '0'*13
            #Fecha de inicio de la utilización
            text += self._formatDate(invoice_investment.use_date)
            #Identificación del bien
            text += self._formatString(tax_line.goods_identification, 17)
            #Blancos
            text += ' '*169
            text += '\r\n'
        assert len(text) == 502 * len(invoice_investment.tax_line_ids), _("The type 2 issued record must be 500 characters long for each Vat registry")
        return text
    ...
    def _get_formatted_other_records(self, cr, uid, report,context=None):
        ...
        if report.investment_goods_prorating:
            for invoice_investment in report.investment:
                file_contents += self._get_formatted_invoice_investment(cr,uid,
                                                          report, invoice_investment)
        else:
            for invoice_investment in report.investment:
                file_contents += self._get_formatted_invoice_received(cr,uid,
                                                      report, invoice_investment)
