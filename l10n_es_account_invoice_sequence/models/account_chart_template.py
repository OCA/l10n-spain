...
class AccountChartTemplate(models.Model):
    ...
    def _create_bank_journals(self, company, acc_template_ref):
        ...
        if self.is_spanish_chart():
            ...
            bank_journals.write(
                {"secure_sequence_id": sequence.id, "refund_sequence": False}
            )
    ...
    def _prepare_all_journals(self, acc_template_ref, company_id, journals_dict=None):
        ...
        for journal_vals in journal_data:
            ...
            journal_vals["invoice_sequence_id"] = journal_seq.id
            if journal_vals["type"] in journal_model._get_invoice_types():
                seq = journal_model._create_secure_sequence(journal_vals)
                if seq:
                    journal_vals["invoice_sequence_id"] = seq.id
                    refund_seq = journal_model._create_secure_sequence(journal_vals)
