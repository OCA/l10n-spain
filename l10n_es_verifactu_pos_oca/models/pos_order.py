...
class PosOrder(models.Model):
    ...
    def action_pos_order_paid(self):
        ...
        refund_orders = self.filtered(
            lambda order: order._is_refund_order()
            and order._is_verifactu_order()
            and not order.verifactu_hash
        )
        for refund_order in refund_orders:
            refund_order.verifactu_registration_date = fields.Datetime.now()
            try:
                refund_order._generate_verifactu_chaining()
            except UserError as e:
                _logger.error(
                    "[ID: %d, REF: %s, INV: %s] Failed to create verifactu "
                    "chaining: %s",
                    refund_order.id,
                    refund_order.pos_reference,
                    refund_order.l10n_es_unique_id,
                    str(e),
                )
        return res

    def refund(self):
        previous_refunds = self.mapped("lines.refund_orderline_ids.order_id")
        action = super().refund()
        new_refunds = (
            self.mapped("lines.refund_orderline_ids.order_id") - previous_refunds
        )
        self._assign_simplified_invoice_to_refunds(new_refunds)
        return action

    def _assign_simplified_invoice_to_refunds(self, refund_orders):
        for refund_order in refund_orders:
            self._assign_simplified_invoice_to_refund(refund_order)

    def _assign_simplified_invoice_to_refund(self, refund_order):
        if not refund_order:
            return (False, False)
        config = refund_order.session_id.config_id
        sequence = config.l10n_es_simplified_invoice_sequence_id if config else False
        if (
            not config
            or not config.is_simplified_config
            or not sequence
            or not refund_order._simplified_limit_check(
                abs(refund_order.amount_total), config.l10n_es_simplified_invoice_limit
            )
        ):
            return (sequence, refund_order.l10n_es_unique_id or False)
        if refund_order.l10n_es_unique_id:
            refund_order.is_l10n_es_simplified_invoice = True
            return (sequence, refund_order.l10n_es_unique_id)
        simplified_number = sequence.next_by_id()
        refund_order.write(
            {
                "l10n_es_unique_id": simplified_number,
                "is_l10n_es_simplified_invoice": True,
            }
        )
        return (sequence, simplified_number)
    ...
    def _get_verifactu_invoice_dict_out(self, cancel=False):
        ...
        if self._is_refund_order():
            refund_type = self.verifactu_refund_type or "I"
            inv_dict["TipoRectificativa"] = refund_type
                ...
                inv_dict["FacturasRectificadas"] = [
                    {
                        "IDFacturaRectificada": {
                            ...
                            "NumSerieFactura": (
                                original_order._get_document_serial_number()
                            ),
                            "FechaExpedicionFactura": (
                                original_order._change_date_format(
    ...
    def _check_verifactu_configuration(self):
        ...
        if not self.fiscal_position_id:
            raise UserError(
                _(
                    ...
                    "cannot be sent to Verifactu because it does not have "
                    "a fiscal position."
                )
                % {
                    "id": self.id,
        ...
        if not self._check_inconsistent_taxes():
            raise UserError(
                _(
                    "[ID: %(id)d, REF: %(ref)s, INV: %(inv)s] The POS order "
                    "cannot be sent to Verifactu because there are some "
                    "inconsistent taxes on lines."
                )
                % {
                    "id": self.id,
                    "ref": self.pos_reference,
            ...
            for order in self.filtered(
                ...
                raise UserError(
                    _(
                        ...
                        "You cannot change the %(fields)s of document already "
                        "registered at VERI*FACTU. You must cancel the document "
                        "and create a new one with the correct value."
                    )
                    % {
                        "id": order.id,
