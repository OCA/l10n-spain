    ...
    const VerifactuOrder = (Order) =>
        ...
        class VerifactuOrder extends Order {
            export_for_printing() {
                ...
                // Generate QR if order is finalized or already paid (for reprints)
                const shouldGenerateQR =
                    !this.is_to_invoice() &&
                    (this.finalized ||
                        (this.locked && ["paid", "done"].includes(this.state)));
                result.verifactu_qr =
                    shouldGenerateQR && this._get_verifactu_qr_code_data();
                return result;
            }
