...
#			self.result_total = {}
	...
	def sort_result(self,accounts):
		...
		while (ind_tab < len(accounts)):
			...
			## on teste la rupture de sￃﾩquence 
			if (tab_rup == 9999):
				tab_rup = ind_tab
			##
			...
			## On va comparer les diffￃﾩrents les old level si il est plus grand que le courant on est dans le cas d'une rupture de sￃﾩquence  
...
#					print "TMP id" + str(tmp_ind) + " Current Level" + str(account_elem['level'])
					...
					new_item['type'] = 2
					# on insￃﾨre ce nouvel ￃﾩlￃﾩment dans la structure
					##
					...
					# on reset l'éléments de position que l'on a inséré pour le résultat final
					tree_struct[str(tmp_ind)]['level'] = 99
 
					tmp_ind -= 1
					tab_rup += 1
					# maintenant que l'on a modifier la rupture finale on va modifier le type du compte prￃﾩcￃﾩdent
					
					
			##
			## on va tester que l'￯﾿ﾃ￯ﾾﾩl￯﾿ﾃ￯ﾾﾩments pr￯﾿ﾃ￯ﾾﾩsents n'est pas d￯﾿ﾃ￯ﾾﾩj￯﾿ﾃ￯ﾾﾠ￯﾿ﾃ￯ﾾﾨ ins￯﾿ﾃ￯ﾾﾩrer dans notre dictionnaire
