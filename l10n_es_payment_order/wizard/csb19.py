...
class Csb19(orm.Model):
    ...
    def _cabecera_presentador_19(self, cr, uid):
        ...
        if len(texto) != 164:
            raise Log(_('Configuration error:\n\nThe line "%s" is not 162 '
                        'characters long:\n%s') %
                      ('Cabecera presentador 19', texto), True)
        return texto
    ...
    def _cabecera_ordenante_19(self, cr, uid, recibo=None):
        ...
        if len(texto) != 164:
            raise Log(_('Configuration error:\n\nThe line "%s" is not 162 '
                        'characters long:\n%s') %
                      ('Cabecera ordenante 19', texto), True)
        return texto
    ...
    def _individual_obligatorio_19(self, cr, uid, recibo):
        ...
        if len(texto) != 164:
            raise Log(_('Configuration error:\n\nThe line "%s" is not 162 '
                        'characters long:\n%s') %
                      ('Individual obligatorio 19', texto), True)
        return texto

    def _individual_opcional_19(self, cr, uid, recibo):
        """Para poner el segundo texto de comunicación (en lugar de nombre,
        ...
        if len(texto) != 164:
            raise Log(_('Configuration error:\n\nThe line "%s" is not 162 '
                        'characters long:\n%s') %
                      ('Individual opcional 19', texto), True)
        return texto
