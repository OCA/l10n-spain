...
class WizardPaymentFileSpain(orm.TransientModel):
    _name = 'wizard.payment.file.spain'
    _columns = {
        'join': fields.boolean(
            'Join payment lines of the same partner and bank account'),
    ...
    def create_payment_file(self, cr, uid, ids, context):
        ...
        try:
            orden = self.pool['payment.order'].browse(cr, uid,
                                                      context['active_id'],
                                                      context)
            if not orden.line_ids:
                raise Log(_('User error:\n\nWizard can not generate export '
                            'file, there are not payment lines.'), True)
            if (orden.create_account_moves == 'direct-payment' and
                    (orden.state != 'open' and orden.state != 'done')):
                raise Log(_('User error:\n\nIf direct payment is selected to '
                            'create the account moves, you should confirm '
                            'payments befores. Creating the files will make '
                            'the payments.'), True)
            # Comprobamos que exista número de C.C. y que tenga 20 dígitos
            if not orden.mode.bank_id:
                raise Log(_('User error:\n\nThe bank account of the company %s'
                            ' is not defined.') % (orden.mode.partner_id.name),
                          True)
                    ...
                    recibos.append({
                        ...
                        'communication': reduce(lambda x, y: x + ' ' +
                                                ...
                                                [l.name + ' ' + l.communication
                                                 for l in lineas], ''),
                        'communication2': reduce(lambda x, y: x + ' ' +
                                                 (y or ''),
                                                 [l.communication2 for l in
                                                  lineas], ''),
                        'date': max([l.date for l in lineas]),
                        'ml_maturity_date': max([l.ml_maturity_date for l in
                                                 lineas]),
                        'create_date': max([l.create_date for l in lineas]),
                        'ml_date_created': max([l.ml_date_created for l in
                                                lineas]),
            ...
            fname = (_('Remittance_%s_%s.txt') %
                     (orden.mode.tipo, orden.reference)).replace('/', '-')
            ...
            attach_id = obj_attachment.create(
                cr, uid, {'name': fname,
                          'datas': file_remesa,
                          'datas_fname': fname,
                          'res_model': 'payment.order',
                          'res_id': orden.id}, context=context)
            log = _("Successfully Exported\n\nSummary:\n Total amount paid: "
                    "%.2f\n Total Number of Payments: %d\n" %
                    (orden.total, len(recibos)))
            self.pool.get('payment.order').set_done(cr, uid, [orden.id],
                                                    context)
