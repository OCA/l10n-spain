    ...
    def _compute_sigaus_amount(self):
        for line in self:
            ...
            if (
                ...
                # Get taxes for SIGAUS product
                taxes = line._get_sigaus_product_taxes(sigaus_product_id)

                # Use compute_all instead of _compute_taxes in Odoo 18
                tax_results = taxes.compute_all(
                    price_unit=price,
                    currency=line.currency_id,
                    quantity=weight,
                    product=sigaus_product_id,
                    partner=line.sale_line.order_partner_id,
                )

                subtotal = tax_results["total_excluded"]
                tax_amount = (
                    tax_results["total_included"] - tax_results["total_excluded"]
                )
                total = tax_results["total_included"]
