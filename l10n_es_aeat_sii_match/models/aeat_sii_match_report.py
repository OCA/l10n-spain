...
class SiiMatchReport(models.Model):
    ...
    number_records = fields.Integer(
        string='Total records', readonly=True)
    ...
    def _get_match_result_values(self, sii_response):
        ...
        summary = {
            'total': len(invoices),
            'both': len(
                [i for i in invoices if i['invoice_location'] == 'both']
            ),
            'sii': len(
                [i for i in invoices if i['invoice_location'] == 'sii']
            ),
            'odoo': len(
                [i for i in invoices if i['invoice_location'] == 'odoo']
            ),
            'correct': len(
            ...
            'no_test': len(
                [i for i in invoices
                 if (i.get('sii_match_state', False) and
                     i['sii_match_state'] == '1')]
            ),
            'in_process': len(
                [i for i in invoices
        ...
        vals = [(0, 0, i) for i in invoices
                if (i['sii_contrast_state'] != 'correct' or
                    i['sii_match_state'] == '4')]
        return vals, summary
    ...
    def _get_invoices_from_sii(self):
        for sii_match_report in self.filtered(
            ...
            serv = self.env['account.invoice'].search(
                [], limit=1)._connect_sii(mapping_key)
            ...
            try:
                ...
                sii_match_report.sii_match_result.mapped(
                    'sii_match_difference_ids').unlink()
...
class SiiMatchResult(models.Model):
    ...
    def _get_selection_sii_match_state(self):
        return self.env['account.invoice'].fields_get(
            allfields=['sii_match_state'])['sii_match_state']['selection']
