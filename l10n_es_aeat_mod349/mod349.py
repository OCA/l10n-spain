...
class l10n_es_aeat_mod349(orm.Model):
    ...
    def button_calculate(self, cr, uid, ids, context=None):
        calculate_obj = self.pool['l10n.es.aeat.mod349.calculate_records']
        calculate_obj._wkf_calculate_records(cr, uid, ids, context=context)
        return True

    def button_recalculate(self, cr, uid, ids, context=None):
        calculate_obj = self.pool['l10n.es.aeat.mod349.calculate_records']
        calculate_obj._calculate_records(cr, uid, ids, context=context)
        return True

    def button_export(self, cr, uid, ids, context=None):
        export_obj = self.pool["l10n.es.aeat.mod349.export_to_boe"]
    ...
    _columns = {
        ...
        'month_selection': fields.selection(MONTH_MAPPING,
                                            'Month',
        ...
        'frequency_change': fields.boolean('Frequency change',
                                           states={'confirmed': [('readonly',
                                                                  True)]}),
        ## Identification
        'contact_name': fields.char("Full Name",
                                    size=40,
                                    help="Must have name and surname.",
                                    states={'calculated': [('required',
                                                            True)],
                                            'confirmed': [('readonly',
                                                           True)]}),
...
class l10n_es_aeat_mod349_partner_record(orm.Model):
    ...
    _columns = {
        ...
        'name': fields.function(get_record_name,
                                method=True,
                                type="char",
                                size="64",
                                string="Name"),
        ...
        'operation_key': fields.selection(OPERATION_KEY,
                                          'Operation key',
                                          required=True),
...
class l10n_es_aeat_mod349_report_add_partner_records(orm.Model):
    """
    Inheritance of 'l10n.es.aeat.mod349' to add
    a relation between partner record and report
    """
    _inherit = 'l10n.es.aeat.mod349.report'

    _columns = {
        'partner_record_ids':
                fields.one2many('l10n.es.aeat.mod349.partner_record',
                                'report_id',
                                'Partner records',
                                ondelete='cascade',
                                states={'confirmed': [('readonly', True)]}),
...
class l10n_es_aeat_mod349_partner_record_detail(orm.Model):
    ...
    _columns = {
        'partner_record_id':
                fields.many2one('l10n.es.aeat.mod349.partner_record',
                                'Partner record',
                                required=True,
                                ondelete='cascade',
                                select=1),
        ...
        'date': fields.related('invoice_id',
                               'date_invoice',
                               type="date",
                               string="Date",
                               readonly=True),
...
class l10n_es_aeat_mod349_partner_record_add_partner_record_details(orm.Model):
    """
    Inheritance of 'l10n.es.aeat.mod349.partner_record' to add a
    relation between
    ...
    _columns = {
        'record_detail_ids':
                fields.one2many('l10n.es.aeat.mod349.partner_record_detail',
                                'partner_record_id',
                                'Partner record detail IDS',
                                ondelete='cascade'),
    }
...
class l10n_es_aeat_mod349_partner_refund(orm.Model):
    ...
    _columns = {
        ...
        'partner_refund_ok':
                    fields.function(_check_partner_refund_line,
                                    method=True,
...
class l10n_es_aeat_mod349_partner_record_add_partner_refund(orm.Model):
    """
    Inheritance of 'l10n.es.aeat.mod349' to add a
    relation between partner refund and report
    """
    _inherit = 'l10n.es.aeat.mod349.report'

    _columns = {
        'partner_refund_ids':
                        fields.one2many('l10n.es.aeat.mod349.partner_refund',
                                        'report_id',
                                        'Partner refund IDS',
                                        ondelete='cascade',
                                        states={'confirmed': [('readonly',
...
class l10n_es_aeat_mod349_partner_refund_add_partner_refund_detail(orm.Model):
    """
    Inheritance of 'l10n.es.aeat.mod349.partner_refund' to add a relation
    between partner refund and partner refund detail
    """
    _inherit = 'l10n.es.aeat.mod349.partner_refund'

    _columns = {
        'refund_detail_ids':
                fields.one2many('l10n.es.aeat.mod349.partner_refund_detail',
                                'refund_id',
                                'Partner refund detail IDS',
                                ondelete='cascade'),
    }
