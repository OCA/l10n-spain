...
class DeliveryCarrier(models.Model):
    ...
    def seur_soap_send(self, service, method, data):
        def trace(title, data):
            _logger.debug(
                "{} {}: {}".format(method, title, etree.tostring(data["envelope"]))
            )
    ...
    def _seur_prepare_create_shipping(self, picking):
        ...
        return {
            ...
            "pesoBulto": (
                (picking.shipping_weight / picking.number_of_packages or 1) or 1
            ),
    ...
    def seur_create_shipping(self, picking):
        ...
        try:
            error = res["mensaje"] == "ERROR" or not res.get("ECB", {}).get(
                "string", []
            )
            if error:
