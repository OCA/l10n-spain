...
class CreateFacturae(orm.TransientModel):
    ...
    def create_facturae_file(self, cr, uid, ids, context):
        ...
        def _parties_facturae(cr, context):
            ...
            def create_administrative_centres_address(address):
                # como hay 3 direcciones, una por centro, así evito duplicidad de código
                texto = ""
                texto += '<AddressInSpain>'
                if address.street:
                    if address.street2:
                ...
                if address.country_id.code_3166:
                    ...
                    raise log
                texto += '</AddressInSpain>'
                # hace falta dirección de contacto???
                """"
                texto += '<ContactDetails>'
                if address.phone:
            ...
            if administrative:
                texto += create_administrative_centres(invoice_partner_address_obj)
                texto += "</AdministrativeCentres>"
