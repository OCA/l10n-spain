...
class TxRedsys(models.Model):
    ...
    def _extract_amount_data(self, payment_data):
        values = super()._extract_amount_data(payment_data)
        if not values:
            payment_date_dir = payment_data and self.merchant_params_json2dict(payment_data)
            amount = payment_date_dir and payment_date_dir.get("Ds_Merchant_Amount", "0")
            if amount:
                values = {
                    "amount": int(amount) / 100,
                    "currency_code": payment_data.get(
                        "Ds_Merchant_Currency", self.company_id.currency_id.name
                    ),
                }
    ...
    @api.model
    def _search_by_reference(self, provider, data):
        """Given a data dict coming from redsys, verify it and
        find the related transaction record."""
        tx = super()._search_by_reference(provider, data)
        if provider != "redsys":
            return tx
