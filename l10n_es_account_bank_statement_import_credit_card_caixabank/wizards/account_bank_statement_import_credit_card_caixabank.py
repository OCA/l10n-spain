...
class AccountBankStatementImport(models.TransientModel):
    ...
    def _process_record_0010(self, line):
        ...
        st_cabecera = {
            ...
            "contrato": line[c5:c6],
            "tarjeta": line[c6:c7],
            "fecha_inicio_pet_rel_op": datetime.strptime(line[c7:c8], "%d%m%Y").date(),
            "fecha_fin_pet_rel_op": datetime.strptime(line[c8:c9], "%d%m%Y").date(),
    ...
    def _process_record_1010(self, line):
        ...
        st_tarjeta = {
            ...
            * float(
            ...
            "importe_operacion_moneda_contrato": -1
            * float(
                "%s.%s"
    ...
    def _parse_file(self, data_file):
            ...
            for contratos in cabecera["contratos"]:
                for tarjetas in contratos["tarjetas"]:
                    ...
                    for operacion in tarjetas["operaciones"]:
                        if operacion["moneda_original_operacion"] not in monedas.keys():
                            monedas[operacion["moneda_original_operacion"]] = self.env[
                                "res.currency"
                            ]
                    ...
                    for operacion in tarjetas["operaciones"]:
                        conceptos = [
                            operacion["nombre_comercio"],
                            "( " + operacion["localidad_comercio"] + " )",
                        ]
                        if (
                            monedas[operacion["moneda_original_operacion"]].name
                            != operacion["moneda_operacion"]
                        ):
                            amount_currency = operacion["importe_origen"]
                            currency_id = monedas[
                                operacion["moneda_original_operacion"]
                            ].id
                        else:
                            amount_currency = 0.0
                            currency_id = False
                        ...
                        vals_line = {
                            ...
                            "amount": operacion["importe_operacion_moneda_contrato"],
        ...
        date_from = self._format_date(
            credit_card_caixabank[0]["fecha_inicio_pet_rel_op"]
        )
        date_to = self._format_date(credit_card_caixabank[0]["fecha_fin_pet_rel_op"])
