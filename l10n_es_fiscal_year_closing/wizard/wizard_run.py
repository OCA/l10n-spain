...
class wizard_run(wizard.interface):
    ...
    def remove_move(self, cr, uid, operation, fyc, data, context):
        ...
        if operation == 'loss_and_profit':
            ...
            pool.get('l10n_es_fiscal_year_closing.fyc').write(cr, uid, fyc.id, { 'loss_and_profit_move_id': None })
        elif operation == 'net_loss_and_profit':
            move = fyc.net_loss_and_profit_move_id
            pool.get('l10n_es_fiscal_year_closing.fyc').write(cr, uid, fyc.id, { 'net_loss_and_profit_move_id': None })
        elif operation == 'close':
            move = fyc.closing_move_id
            pool.get('l10n_es_fiscal_year_closing.fyc').write(cr, uid, fyc.id, { 'closing_move_id': None })
        elif operation == 'open':
            move = fyc.opening_move_id
            pool.get('l10n_es_fiscal_year_closing.fyc').write(cr, uid, fyc.id, { 'opening_move_id': None })
        else:
    ...
    def _run(self, db_name, uid, data, context=None):
        ...
        try:
            ...
            if data.get('cancel_mode'):
                ...
                wf_service.trg_validate(uid, 'l10n_es_fiscal_year_closing.fyc', data['id'], 'cancel', cr)

            # Read the object
            fyc = pool.get('l10n_es_fiscal_year_closing.fyc').browse(cr, uid, data['id'], context=context)
...
wizard_run('l10n_es_fiscal_year_closing.wizard_run')
...
wizard_cancel('l10n_es_fiscal_year_closing.wizard_cancel')
