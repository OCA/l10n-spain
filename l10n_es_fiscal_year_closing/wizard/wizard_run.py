...
class CancelFyc(orm.TransientModel):
    ...
    _columns = {
        'delete_pyg': fields.boolean('Delete P&L account move',
                                     readonly=True),
        'delete_net_pyg': fields.boolean('Delete net P&L account move',
                                         readonly=True),
        'delete_close': fields.boolean('Delete closing account move',
                                       readonly=True),
        'delete_open': fields.boolean('Delete opening account move',
                                      readonly=True),
    }
    ...
    def default_get(self, cr, uid, field_list, context=None):
        ...
        if context.get('active_id'):
            ...
            fyc = fyc_obj.browse(cr, uid, context['active_id'])
    ...
    def _remove_move(self, cr, uid, move, context):
        ...
        if reconcile_ids:
            ...
            orm.Model.unlink(self.pool['account.move.reconcile'],
        ...
        self.pool['account.move.line'].unlink(
            cr, uid, line_ids, context=context, check=False)
...
class ExecuteFyc(orm.TransientModel):
    ...
    def _check_invalid_period_moves(self, cr, uid, fyc, context):
        ...
        account_move_ids = move_obj.search(
        ...
        if len(account_move_ids):
            ...
            str_invalid_period_moves = \
                '\n'.join(['id: %s, date: %s, number: %s, ref: %s' %
                           (move.id, move.date, move.name, move.ref) for move
                           in invalid_period_moves])
    ...
    def _check_draft_moves(self, cr, uid, fyc, context):
        ...
        draft_move_ids = move_obj.search(
            cr, uid, [('period_id', 'in', period_ids),
    ...
    def _check_unbalanced_moves(self, cr, uid, fyc, context):
        ...
        for move in move_obj.browse(cr, uid, account_move_ids, context):
            ...
            if (round(abs(amount),
                      decimal_precision_obj.precision_get(cr, uid,
                                                          'Account')) > 0):
        ...
        if len(unbalanced_moves):
            str_unbalanced_moves = \
                '\n'.join(['id: %s, date: %s, number: %s, ref: %s'
                           % (move.id, move.date, move.name, move.ref) for
                           move in unbalanced_moves])
            raise orm.except_orm(
                _('Error'),
    ...
    def create_closing_move(self, cr, uid, operation, fyc, context):
        ...
        assert operation in ('loss_and_profit', 'net_loss_and_profit',
                             'close'), _("The operation must be a supported "
                                         "one")
        ...
        return self._create_closing_move(
            cr, uid, account_mapping_ids, period_ids, description, date,
            period_id, journal_id, fyc.company_id.id,
            fyc.closing_fiscalyear_id.id, context)
