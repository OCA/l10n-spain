...
class cancel_fyc(osv.osv_memory):
    ...
    def cancel_run(self, db_name, uid, data, context=None):
        cr=db_name
        try:
            pool = pooler.get_pool(cr.dbname)
...
class execute_fyc(osv.osv_memory):
    ...
    def create_opening_move(self, cr, uid, operation, fyc, context):
        ...
        if operation == 'open':
            ...
            closing_move_id = fyc.closing_move_id.id
        else:
    ...
    def execute_run(self, db_name, uid, data, context=None):
        """
        ...
        try:
            ...
            if total_operations > 0:
                ...
                #
                if (not fyc.create_loss_and_profit) and fyc.loss_and_profit_move_id:
                    move_id = self.remove_move(cr, uid, 'loss_and_profit', fyc, context)
