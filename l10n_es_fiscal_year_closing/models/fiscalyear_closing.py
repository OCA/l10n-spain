...
#    Copyright (C) 2010 Pexego Sistemas Informáticos.
...
class FiscalyearClosingAccountMapping(orm.AbstractModel):
    ...
    _columns = {
        ...
        'fyc_id': fields.many2one(
            'account.fiscalyear.closing', 'Fiscal Year Closing',
...
class FiscalyearClosing(orm.Model):
    ...
    _columns = {
        ...
        'lp_account_mapping_ids': fields.one2many(
            'account.fiscalyear.closing.lp_account_map', 'fyc_id',
            'Account mappings'),
        ...
        'nlp_account_mapping_ids': fields.one2many(
            'account.fiscalyear.closing.nlp_account_map', 'fyc_id',
            'Account mappings'),
        # Closing options
        ...
        'c_account_mapping_ids': fields.one2many(
            'account.fiscalyear.closing.c_account_map', 'fyc_id', 'Accounts'),
        # Opening options
    ...
    def _get_opening_fiscalyear_id(self, cr, uid, context):
        ...
        closing_fy = fiscalyear_obj.read(cr, uid, closing_fy_id,
                                         ['date_stop'], context)
    ...
    _constraints = [
        (_check_duplicate,
         _('There is already a fiscal year closing with the same opening or '
           'ending fiscal year.'), [])
    ]
        ...
        for fyc in self.read(cr, uid, ids, ['state'], context=context):
            if fyc['state'] in ('in_progress', 'done'):
                raise orm.except_orm(
                    _('Error!'),
                    _('You cannot delete a fiscal year closing which is in '
                      'progress or done. You have to cancel it first.'))
        return super(FiscalyearClosing, self).unlink(cr, uid, ids,
                                                     context=context)
    ...
    def _get_defaults(self, cr, uid, fyc, context=None):
        ...
        vals = {
            ...
            'lp_account_mapping_ids': self._get_lp_account_mapping(cr, uid,
                                                                   fyc,
                                                                   context),
            # Net L&P options
            ...
            'c_account_mapping_ids': self._get_c_account_mapping(cr, uid,
                                                                 fyc, context),
            # Opening options
...
# Workflow actions ---------------------------------------------------------
    ...
    def action_confirm(self, cr, uid, ids, context=None):
        ...
        for fyc in self.browse(cr, uid, ids, context):
            ...
            for move in moves:
                ...
                #       reconcile' ones, to prevent future problems (the user
                #       may change the reconcile option of an account in the
                #       future)
                ...
                cr.execute(
                    ...
                    SET reconcile_id = %s
                    WHERE id in %s""", (r_id, tuple(line_ids),))
            ...
            #       period!", so we have to do it on SQL level :(
    ...
    def action_recover(self, cr, uid, ids, context=None):
        ...
        for item_id in ids:
            wf_service.trg_delete(uid, 'account.fiscalyear.closing', item_id,
                                  cr)
            wf_service.trg_create(uid, 'account.fiscalyear.closing', item_id,
                                  cr)
            wf_service.trg_validate(uid, 'account.fiscalyear.closing', item_id,
                                    'draft', cr)
        return True
