...
class L10nEsAeatMod303ExportToBoe(models.TransientModel):
    ...
    def _get_formatted_main_record(self, report):
        lines = report._get_report_lines()
        res = ''
        # IVA devengado
        ...
        # Regularización inversiones por aplicación del porcentaje def de
        # prorrata
    ...
    def _get_formatted_other_records(self, report):
        ...
        if report.cuenta_ingreso_id and report.ingresar:
            ...
            if not (len(ccc) == 20 and ccc.isdigit()):
                raise exceptions.Warning(
                    _('Warning'), _("CCC de ingreso no válido %s") % ccc)
        res += self._formatString(ccc, 20)  # no hay devolución
