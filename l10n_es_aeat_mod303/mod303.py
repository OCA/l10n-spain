...
class L10nEsAeatMod303Report(orm.Model):
    ...
    _columns = {
        ...
        'atribuible_estado': fields.float(
            "Atribuible a la Administración", readonly=True),  # 40
        ...
        'regularizacion_anual': fields.float(
            "Regularización anual", help="En la última autoliquidación del "
        ...
        'resultado_casilla_46': fields.float(
            "Resultado", help="Atribuible a la Administración [40] - Cuotas "
            "a compensar [41] + Regularización anual [45]""",
            readonly=True),  # 46
        'previus_result': fields.float(
            "A deducir", help="Resultado de la anterior o anteriores del "
        ...
        for mod303 in self.browse(cr, uid, ids, context=context):
            ...
            if not account_period_id:
                raise orm.except_orm(
                    _('El periodo seleccionado no coincide con los periodos '
                      'del ejercicio fiscal: '), dec_year)
    ...
    def _get_report_lines(self, cr, uid, ids, context=None):
        ...
        try:
            lines = generated_report._get_lines(
                'invoices', company_id=mod303.company_id.id)
        except TypeError:
            ...
            lines = generated_report._get_lines(
                'invoices', False, company_id=mod303.company_id.id)
    ...
    def calculate(self, cr, uid, ids, context=None):
        for mod303 in self.browse(cr, uid, ids, context=context):
            ...
            vals = {
                ...
                'ingresar': (resultado_liquidacion if
                             resultado_liquidacion > 0 else 0),
            }
            if (mod303.regularizacion_anual > 0 and not
                    (mod303.period == "4T" and mod303.period == "12")):
                self.log(cr, uid, mod303.id,
                         _("El valor añadido para la regularizacion anual no "
                           "se ha tenido en cuenta por no ser un periodo de "
                           "cierre (12 o 4T)"), context=context)
