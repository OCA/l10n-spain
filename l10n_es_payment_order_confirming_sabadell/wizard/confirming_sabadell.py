...
from openerp.addons.l10n_es_payment_order.wizard.converter import PaymentConverterSpain
...
class ConfirmingSabadell(object):
    ...
    def _sab_registro_01(self):
        if self.order.date_prefered == "due":
            ...
            if not fecha_planificada:
                raise Log(
                    _(
                        "Error: Fecha planificada no establecida en \
                        la Orden de pago."
                    )
                )
            else:
        ...
        if not ordenante:
            raise Log(
                _(
                    "Error: Propietario de la cuenta no \
    ...
    def _sab_registro_03(self, line):
        ...
        if not ciudad_pro:
            raise Log(
                _(
                    "Error: El Proveedor %s no tiene establecida la \
                    Ciudad."
                )
                % line["partner_id"]["name"]
            )
