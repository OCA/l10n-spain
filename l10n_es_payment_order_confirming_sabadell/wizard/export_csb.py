...
class BankingExportCsbWizard(models.TransientModel):
    ...
    def _check_company_bank_account(self, payment_order):
        """Don't make this check for Confirming Sabadell."""
        if not payment_order.mode.is_conf_sabadell:
    ...
    def _get_csb_exporter(self, payment_order):
        if payment_order.mode.type.code == "conf_sabadell":
            csb = ConfirmingSabadell(self.env)
        else:
            csb = super(BankingExportCsbWizard, self)._get_csb_exporter(payment_order)
        return csb
